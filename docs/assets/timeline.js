...
function showTimelineDetail(id){const e=TDATA?.events?.find(x=>x.id===id),box=document.getElementById('timelineDetail');if(!e||!box)return;TFOCUS=e.id;const summary=(e.summary||e.description||'').trim(),studyLinks=(e.study_topics||[e.study_topic]).filter(Boolean).map(topic=>`<a class="timeline-miracle-link" href="topics.html?topic=${encodeURIComponent(topic)}">Study: ${topic} →</a>`).join('');box.innerHTML=`<b>${e.label} · ${e.title}</b><span>${summary||'No extended summary is currently available for this event.'}</span>${e.source?`<small>Source / discovery layer: ${e.source}</small>`:''}${e.registry_query?`<a class="timeline-miracle-link" href="miracle-registry.html?q=${encodeURIComponent(e.registry_query)}">Open related Registry records →</a>`:''}${studyLinks}`}
