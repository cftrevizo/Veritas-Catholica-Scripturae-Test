...
function renderTimeline(){
  ...
  ev.sort((a,b)=>a.year-b.year).forEach(e=>{
    ...
    const t=S('text',{x:xx+8,y:yy-8,class:'timeline-event-label stacked'});t.dataset.baseX=String(xx);t.dataset.baseY=String(yy);t.dataset.side='right';t.dataset.eventId=e.id;t.textContent=e.title;g.appendChild(t);
    t.addEventListener('click',openDetail);t.addEventListener('pointerenter',eventSummary);t.addEventListener('pointermove',eventSummary);t.addEventListener('pointerleave',hideTip);
  });
