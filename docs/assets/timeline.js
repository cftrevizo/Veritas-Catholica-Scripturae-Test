...
let TDATA=null, TSCOPES=new Set(['ot','apostolic','church']), TIMPORTS=new Set(['foundational','major']), TCATS=new Set(), TCAT_ALL=true, TFOCUS=null, TLAYOUT=null;
const TLEVEL={foundational:0,major:1,significant:2,detailed:3,source:4};
const TCAT_LABEL={biblical_events:'Biblical Events',patriarchs:'Patriarchs',covenants:'Covenants',kings:'Kings / Israel & Judah',prophets:'Prophets',empires:'Empires / World Context',messianic:'Messianic',jesus:'Jesus Christ',apostles:'Apostles',fathers:'Fathers & Writings',councils:'Councils & Magisterium',doctrine:'Doctrine',schisms:'Schisms',reforms:'Reforms',martyrs:'Persecution / Martyrs',world:'World Context',scripture:'Scripture / Canon',missions:'Missions / Saints',miracles:'Miracles',apparitions:'Marian Apparitions & Devotions',eucharistic:'Eucharistic Signs',relics:'Relics & Sacred Images',saints:'Saints & Causes',discernment:'Discernment & Governance'};
const SCOPE_CATS={ot:['biblical_events','patriarchs','covenants','kings','prophets','empires','messianic','scripture'],apostolic:['jesus','apostles','fathers','councils','martyrs','world','scripture','messianic'],church:['fathers','councils','doctrine','schisms','reforms','martyrs','scripture','missions','world','miracles','apparitions','eucharistic','relics','saints','discernment']};
function showTimelineDetail(id){const e=TDATA?.events?.find(x=>x.id===id),box=document.getElementById('timelineDetail');if(!e||!box)return;TFOCUS=e.id;document.querySelectorAll('.timeline-event,.timeline-event-hit,.timeline-event-label').forEach(n=>n.classList.toggle('timeline-selected',n.dataset.eventId===id));const summary=(e.summary||e.description||'').trim(),studyLinks=(e.study_topics||[e.study_topic]).filter(Boolean).map(topic=>`<a class="timeline-miracle-link" href="topics.html?topic=${encodeURIComponent(topic)}">${topic} →</a>`).join(''),studyPanel=`<div class="timeline-detail-links"><b>Study connections</b>${studyLinks||'<span class="timeline-no-connections">No linked Study Topic is indexed for this event yet.</span>'}</div>`;box.innerHTML=`<b>${e.label} · ${e.title}</b><span>${summary||'No extended summary is currently available for this event.'}</span>${e.source?`<small>Source / discovery layer: ${e.source}</small>`:''}${e.registry_query?`<div class="timeline-detail-links"><b>Registry connection</b><a class="timeline-miracle-link" href="miracle-registry.html?q=${encodeURIComponent(e.registry_query)}">Open related Registry records →</a></div>`:''}${studyPanel}`}
window.vcsShowTimelineDetail=showTimelineDetail;
...
function timelineControls(){let cats=activeTimelineCats();if(!TCAT_ALL)TCATS=new Set([...TCATS].filter(c=>cats.includes(c)));document.getElementById('timelineCategories').innerHTML=cats.map(c=>`<button class="timeline-chip ${TCAT_ALL||TCATS.has(c)?'active':''}" data-timeline-category="${c}" aria-pressed="${TCAT_ALL||TCATS.has(c)}">${TCAT_LABEL[c]||c}</button>`).join('');document.querySelectorAll('[data-timeline-category]').forEach(b=>b.onclick=()=>{if(TCAT_ALL){TCAT_ALL=false;TCATS=new Set(cats)}let c=b.dataset.timelineCategory;TCATS.has(c)?TCATS.delete(c):TCATS.add(c);if(TCATS.size===cats.length){TCAT_ALL=true;TCATS.clear()}timelineControls();renderVisual();scheduleFitVisual()});syncScopeButtons();syncImportanceButtons()}
function timelineFiltered(){if(!TDATA)return[];return TDATA.events.filter(e=>TSCOPES.has(e.scope)&&TIMPORTS.has(e.importance)&&(TCAT_ALL||TCATS.has(e.category)||e.tags?.some(t=>TCATS.has(t))))}
...
function renderTimeline(){
  ...
  ev.sort((a,b)=>a.year-b.year).forEach(e=>{
    ...
    const openDetail=q=>{showTimelineDetail(e.id);if(q.pointerType==='touch')eventSummary(q)};
    ...
    hit.dataset.cx=String(xx);hit.dataset.cy=String(yy);hit.dataset.eventId=e.id;
    ...
    node.addEventListener('focus',eventSummary);node.addEventListener('keydown',q=>{if(q.key==='Enter'||q.key===' '){q.preventDefault();showTimelineDetail(e.id)}});
    node.addEventListener('blur',hideTip);
    g.appendChild(hit);g.appendChild(node);
    const t=S('text',{x:xx+8,y:yy-8,class:'timeline-event-label stacked'});t.dataset.baseX=String(xx);t.dataset.baseY=String(yy);t.dataset.side='right';t.dataset.eventId=e.id;t.tabIndex=0;t.setAttribute('role','button');t.textContent=e.title;g.appendChild(t);
    t.addEventListener('click',openDetail);t.addEventListener('keydown',q=>{if(q.key==='Enter'||q.key===' '){q.preventDefault();showTimelineDetail(e.id)}});t.addEventListener('pointerenter',eventSummary);t.addEventListener('pointermove',eventSummary);t.addEventListener('pointerleave',hideTip);
  });
  ...
  if(TFOCUS)showTimelineDetail(TFOCUS);
  requestAnimationFrame(renderTimelineGuides);
}
function initTimelineControls(){document.querySelectorAll('[data-timeline-scope]').forEach(b=>b.onclick=()=>{let s=b.dataset.timelineScope;if(s==='all'){TSCOPES=new Set(['ot','apostolic','church'])}else if(TSCOPES.has(s)){if(TSCOPES.size>1)TSCOPES.delete(s)}else TSCOPES.add(s);TCAT_ALL=true;TCATS.clear();timelineControls();renderVisual();scheduleFitVisual()});document.querySelectorAll('[data-timeline-importance]').forEach(b=>b.onclick=()=>{let v=b.dataset.timelineImportance;if(TIMPORTS.has(v)){if(TIMPORTS.size>1)TIMPORTS.delete(v)}else TIMPORTS.add(v);syncImportanceButtons();renderVisual();scheduleFitVisual()});document.getElementById('timelineDeselectCats')?.addEventListener('click',()=>{TCAT_ALL=false;TCATS.clear();timelineControls();renderVisual();scheduleFitVisual()});document.getElementById('timelineShowAllCats')?.addEventListener('click',()=>{TCAT_ALL=true;TCATS.clear();timelineControls();renderVisual();scheduleFitVisual()});timelineControls()}
