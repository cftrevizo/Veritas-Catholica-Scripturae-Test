...
function renderTimeline(){
  ...
  ev.sort((a,b)=>a.year-b.year).forEach(e=>{
    ...
    const openDetail=q=>{TFOCUS=e.id;document.getElementById('timelineDetail').innerHTML=`<b>${e.label} · ${e.title}</b><span>${summary||'No extended summary is currently available for this event.'}</span>${e.source?`<small>Source / discovery layer: ${e.source}</small>`:''}${e.registry_query?`<a class="timeline-miracle-link" href="miracle-registry.html?q=${encodeURIComponent(e.registry_query)}">Open related Registry records →</a>`:''}${e.study_topic?`<a class="timeline-miracle-link" href="topics.html?topic=${encodeURIComponent(e.study_topic)}">Study related topic →</a>`:''}`; if(q.pointerType==='touch')eventSummary(q)};
