...
(function(){
  ...
  const node=(g,x,y,title,sub,topic,kind='concept')=>{
    ...
    const open=()=>location.href=`topics.html?topic=${encodeURIComponent(topic)}`;
    group.addEventListener('click',open);group.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open()}});
    group.addEventListener('pointerenter',e=>tipV(e,`<b>${title}</b><br>${sub}<br><span class="tip-note">Open related Study Topics →</span>`));group.addEventListener('pointerleave',hideTip);g.appendChild(group);
  };
  ...
  window.renderSalvationHistory=function(){const {svg,g}=clearV();svg.setAttribute('viewBox','0 0 1400 760');svg.style.height='auto';
    ...
    const b=document.createElementNS(NS,'text');b.setAttribute('x',700);b.setAttribute('y',600);b.setAttribute('class','concept-caption');b.textContent='Prototype map · each stage will later connect Scripture, Catechism, Timeline, and related Study Topics';g.appendChild(b);document.getElementById('visualMeta').innerHTML='<b>Salvation History · concept prototype</b> · the Catholic 73-book canon read as one unfolding account of God’s saving work';
  };
